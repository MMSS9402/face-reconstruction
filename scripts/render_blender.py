"""
Blender single mask renderer.

DECA face-only .obj를 단일 mask 이미지로 렌더링한다.
...
def setup_lighting(obj):
    """Neutral studio lighting for a reusable face mask render."""
    bbox = [obj.matrix_world @ Vector(c) for c in obj.bound_box]
    xs = [v.x for v in bbox]
    ys = [v.y for v in bbox]
    zs = [v.z for v in bbox]
    ...
    add_light("Key", 1.6, (1.0, 1.0, 1.0),
              (cx + d*0.4, y_min - d*0.9, cz + d*0.35), size_mult=4.0)

    add_light("Fill", 0.7, (0.95, 0.97, 1.0),
              (cx - d*0.5, y_min - d*0.7, cz + d*0.1), size_mult=5.0)

    add_light("Top", 0.4, (1.0, 0.99, 0.97),
...
def create_mask_material(obj):
    mat = bpy.data.materials.new(name="MaskSurface")
    mat.use_nodes = True
    ...
    if vc_attr:
        ...
        mix_rgb.inputs[6].default_value = (0.78, 0.74, 0.72, 1.0)
        links.new(gamma.outputs["Color"], mix_rgb.inputs[7])

        hsv = nodes.new("ShaderNodeHueSaturation")
        hsv.location = (-200, 0)
        hsv.inputs["Hue"].default_value = 0.5
        hsv.inputs["Saturation"].default_value = 0.7
        hsv.inputs["Value"].default_value = 0.95
        links.new(mix_rgb.outputs[2], hsv.inputs["Color"])

        links.new(hsv.outputs["Color"], bsdf.inputs["Base Color"])
    else:
        bsdf.inputs["Base Color"].default_value = (0.76, 0.72, 0.70, 1.0)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Blender single mask renderer")
...
def render_mask(obj, output_path, samples, resolution):
    print(f"\n{'='*60}")
    print("  Rendering: mask")
    print(f"  Output: {output_path}")
    print(f"{'='*60}")

    create_mask_material(obj)
...
def main():
    ...
    out_path = output_dir / "mask_render.png"
    render_mask(obj, out_path, args.samples, args.resolution)

    print(f"\nRender complete. Output: {output_dir}")
